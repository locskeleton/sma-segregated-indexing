...
`J01 sync_fo (mirror holdings + cash per-KH từ snapshot T_CUSTOMER_HOLDING_DAILY; biến động/ngày suy ra on-demand) → J07 compute (MTM→NAV→PnL→Unit, roll-forward, perf per-KH) → J11 SI agg → J12 SI index → J13 reconcile (cổng) → J14 snapshot`.
(FO đồng bộ holdings+cash TỪNG KH cuối ngày — SDI không quản lý từng lệnh khớp. Snapshot holdings dated giữ trong T_CUSTOMER_HOLDING_DAILY (audit/tái dựng); biến động net/ngày suy ra on-demand khi cần. Cashflow event chỉ dùng cho CF_t.)
...
> Chưa implement (mở rộng): ingestion file FO → `T_CUSTOMER_HOLDING_DAILY` + `T_FO_CASH_SYNC` + `T_CUSTOMER_FEE_INCOME` (cổ tức/phí, `BULK INSERT`), J15 publish→Asset, read procs `SP_GET_*` (FR-01..06), MWR (Modified Dietz set-based / XIRR qua SQL CLR), partition/columnstore prod (gồm `T_CUSTOMER_NAV_DAILY` CCI).
