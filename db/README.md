...
11_JOB.sql         -- [near-RT FO][BẢN TỐI GIẢN: đã gỡ lease/singleton/hạn tươi/SP_JOB_RECOVER — xem banner đầu docs/SDI-nearrt-fo-snapshot-design.md] khung job chạy nền GENERIC (T_JOB_DEFINITION/T_JOB_RUN + SP_SET_JOB_SCHEDULE (cổng đổi chu kỳ 15'/30'/1h — phía app PHẢI gọi qua JobConfigService để nạp lại cache SDI:JOB:CFG ngay, vì bộ quét đọc chu kỳ từ cache chứ không từ DB — đổi cấu hình là DỌN luợt chờ của cấu hình cũ, có trigger TR_JOB_DEFINITION_PURGE_PENDING làm lưới chặn) + SP_JOB_CLAIM_SLOT (CỔNG DUY NHẤT — gộp enqueue+claim: dòng sinh ra ĐÃ ở RUNNING, bộ quét KHÔNG ghi DB)/_MARK_SKIPPED/_HEARTBEAT/_COMPLETE/_RECOVER/_PURGE + SP_GET_SCHEDULABLE_JOBS/SP_GET_JOB_STATUS + UDF_JOB_SLOT_AT/UDF_JOB_FIRE_KEY (bản THAM CHIẾU của phép tính mốc — pod tự tính để khỏi quét DB mỗi 10s, DB đối chiếu lại, lệch thì err=20)) — chống trùng nhiều pod bằng UQ(job_code,fire_key) + UPDATE có điều kiện; guard giờ giấc gồm HAI phép kiểm, cả hai neo vào MỐC SLOT (C_SLOT_AT): UDF_JOB_IN_WINDOW (mốc có trong khung [09:00,15:00] đóng hai đầu + đúng ngày GD) và C_MAX_DELAY_SEC (hạn tươi: lượt 15:00 hết hiệu lực lúc 15:10). KHÔNG có cửa hậu ignore_window, KHÔNG có khái niệm ân hạn. Chuông cửa = Kafka topic riêng sdi.job.notify (key=fireKey, value={code,slot,key}); Redis chỉ LỌC TRƯỚC (SET NX theo mốc) để bộ quét không chạm DB mỗi nhịp — Redis LỖI thì bộ quét DỪNG nhịp, KHÔNG có đường vòng quét DB. Kèm tầng nghiệp vụ FO snapshot: SP_GET_FO_SNAPSHOT_SCOPE / SP_INGEST_FO_SNAPSHOT_RT (ghi C_SRC='RT') / SP_RT_MASTER_AGG / SP_GET_PM_RT_OVERVIEW. Chạy sau 02.
...
12_JOB_SMOKE.sql   -- smoke khung job + snapshot RT (71 ca: 2-pod tranh job, lease/zombie, 4 tầng guard giờ GD, RT không đè EOD, RT không lọt vào err=12/phí/báo cáo; khối (C): đổi chu kỳ ⇒ dọn lượt cũ, trigger backstop, job quá giờ KHÔNG chạy lại, chưa-cấu-hình/xoá-cấu-hình, mất message vẫn nhặt lại) — cần 11
