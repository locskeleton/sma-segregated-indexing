...
DELETE FROM T_SI_BALANCE         WHERE C_MASTER_CODE='M1';
DELETE FROM T_SI_CURRENT         WHERE C_MASTER_CODE='M1';
DELETE FROM T_SI_PORTFOLIO           WHERE C_MASTER_CODE='M1';
DELETE FROM T_MASTER_BALANCE     WHERE C_MASTER_CODE='M1';
...
-- [thin-layer] master daily_return = AUM-weighted Σ(AUMᵢ·rᵢ)/ΣAUMᵢ → composite ∏(1+r) endpoint ≈ .08
INSERT T_MASTER_BALANCE
 (C_BUSINESS_DATE,C_MASTER_CODE,C_CASH,C_AUM,C_DAILY_RETURN,C_CASH_IN,C_CASH_OUT,C_TOTAL_ACCOUNT) VALUES
 (@D1,'M1',16000000,370000000,NULL,0,0,3),
 (@D2,'M1',16000000,390000000,0.052500,50000000,0,3),
 (@D3,'M1',16000000,400000000,0.026250,0,20000000,3);

-- master current snapshot (cash drag = 16/400 = .04)
INSERT T_MASTER_CURRENT
 (C_MASTER_CODE,C_CASH,C_AUM,C_TOTAL_ACCOUNT,C_LAST_BUSINESS_DATE)
 VALUES ('M1',16000000,400000000,3,@D3);
...
-- per-KH current (AUM = C_LAST_AUM; tien = cash). [thin-layer] gỡ unit/UP.
INSERT T_SI_CURRENT
 (C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE,C_CASH,C_LAST_AUM,C_STATUS,C_LAST_BUSINESS_DATE) VALUES
 ('S1','C1','M1',10000000,100000000,'ACTIVE',@D3),
 ('S2','C2','M1', 4000000,200000000,'ACTIVE',@D3),
 ('S3','C3','M1', 2000000,100000000,'ACTIVE',@D3);

-- per-KH daily balance (AUM + daily_return). D1 return NULL. Compound: S1=.08 S2=.06 S3=.12 → AUM-weighted .08
INSERT T_SI_BALANCE (C_BUSINESS_DATE,C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE,C_AUM,C_DAILY_RETURN) VALUES
 (@D1,'S1','C1','M1',100000000,NULL),
 (@D2,'S1','C1','M1',105000000,0.050000),
 (@D3,'S1','C1','M1',108000000,0.028571),
 (@D1,'S2','C2','M1',200000000,NULL),
 (@D2,'S2','C2','M1',206000000,0.030000),
 (@D3,'S2','C2','M1',212000000,0.029126),
 (@D1,'S3','C3','M1',100000000,NULL),
 (@D2,'S3','C3','M1',110000000,0.100000),
 (@D3,'S3','C3','M1',112000000,0.018182);
...
SELECT C_BUSINESS_DATE,C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE FROM T_SI_BALANCE WHERE C_MASTER_CODE='M1';
