...
INSERT T_MASTER_NAV_BALANCE
 (C_BUSINESS_DATE,C_MASTER_CODE,C_CASH,C_AUM,C_UNIT,C_UNIT_PRICE,C_DAILY_PNL,C_DAILY_RETURN,C_CASH_IN,C_CASH_OUT,C_TOTAL_ACCOUNT) VALUES
 (@D1,'M1',16000000,370000000,37000,10000,0,NULL,0,0,3),
 (@D2,'M1',16000000,390000000,37000,10541,20000000,0.050000,50000000,0,3),
 (@D3,'M1',16000000,400000000,37000,10752,10000000,0.020000,0,20000000,3);

-- master current snapshot (cash drag = 16/400 = .04)
INSERT T_MASTER_NAV_CURRENT
 (C_MASTER_CODE,C_CASH,C_AUM,C_UNIT,C_LAST_UNIT_PRICE,C_TOTAL_ACCOUNT,C_LAST_BUSINESS_DATE)
 VALUES ('M1',16000000,400000000,37000,10752,3,@D3);
...
-- per-KH current (AUM end = LAST_NAV; tien = cash). [BRD] phí QL đã trừ trong NAV ⇒ AUM = NAV
INSERT T_SI_NAV_CURRENT
 (C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE,C_UNIT,C_CASH,C_LAST_NAV,C_LAST_UNIT_PRICE,C_STATUS,C_LAST_BUSINESS_DATE) VALUES
 ('S1','C1','M1',10000,10000000,100000000,10800,'ACTIVE',@D3),
 ('S2','C2','M1',20000, 4000000,200000000,10600,'ACTIVE',@D3),
 ('S3','C3','M1', 9000, 2000000,100000000,11200,'ACTIVE',@D3);

-- per-KH daily balance (unit_price + daily_return). D1 return NULL.
INSERT T_SI_NAV_BALANCE (C_BUSINESS_DATE,C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE,C_NAV,C_UNIT,C_UNIT_PRICE,C_DAILY_PNL,C_DAILY_RETURN) VALUES
 (@D1,'S1','C1','M1',100000000,10000,10000,0,NULL),
 (@D2,'S1','C1','M1',105000000,10000,10500,5000000,0.050000),
 (@D3,'S1','C1','M1',108000000,10000,10800,3000000,0.028571),
 (@D1,'S2','C2','M1',200000000,20000,10000,0,NULL),
 (@D2,'S2','C2','M1',206000000,20000,10300,6000000,0.030000),
 (@D3,'S2','C2','M1',212000000,20000,10600,6000000,0.029126),
 (@D1,'S3','C3','M1',100000000, 9000,10000,0,NULL),
 (@D2,'S3','C3','M1',110000000, 9000,11000,10000000,0.100000),
 (@D3,'S3','C3','M1',112000000, 9000,11200,2000000,0.018182);
...
-- SP_EOD_TE_ACCUM scope theo T_EOD_WORK @p_d (rerun-safe) → smoke seed THẲNG NAV_BALANCE nên phải seed work
--   cho 3 SI × 3 phiên (production: compute populate work trước TE; ở đây mô phỏng).
INSERT T_EOD_WORK (C_BUSINESS_DATE,C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE)
SELECT C_BUSINESS_DATE,C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE FROM T_SI_NAV_BALANCE WHERE C_MASTER_CODE='M1';
