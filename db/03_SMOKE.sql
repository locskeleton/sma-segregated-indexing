...
INSERT T_SI_BALANCE (C_BUSINESS_DATE,C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE,C_AUM,C_CASH,C_CASH_AVAILABLE,
                     C_DIVIDEND_PENDING,C_SELL_PENDING) VALUES
 ('2026-09-01','RP1','RC1','RM1',1000000000,200000000,150000000, 20000000, 30000000),  -- tiền mặt = 200−20−30 = 150tr
 ('2026-09-01','RP2','RC1','RM2', 500000000, 50000000, 50000000,        0,        0),
 ('2026-09-01','RP3','RC2','RM1', 300000000, 30000000, 30000000,  5000000,        0);
...
CREATE TABLE #rp (C_STT BIGINT, C_BUSINESS_DATE DATE, C_TKCK VARCHAR(32), C_CUST_CODE VARCHAR(10),
    ...
    C_SELL_PENDING DECIMAL(20,0), C_CASH_AVAILABLE DECIMAL(20,0), C_MASTER_CODE VARCHAR(20), C_MASTER_NAME NVARCHAR(200),
...
DECLARE @nl1 INT=(SELECT COUNT(*) FROM #rp WHERE C_CASH_TOTAL <> C_CASH_ON_HAND + C_DIVIDEND_PENDING + C_SELL_PENDING);
DECLARE @oh1 DECIMAL(20,0)=(SELECT C_CASH_ON_HAND FROM #rp WHERE C_SI_ACCOUNT='RP1');
DECLARE @dv1 DECIMAL(20,0)=(SELECT C_DIVIDEND_PENDING FROM #rp WHERE C_SI_ACCOUNT='RP1');
IF @ecP2=0 AND @n1=3 AND @ck1=800000000 AND @pc1=0.200000 AND @tk1='068C111111'
   AND @nl1=0 AND @oh1=150000000 AND @dv1=20000000
   PRINT '  OK không lọc: 3 dòng · CK=AUM−tiền=800tr · %tiền=0.2 · TKCK từ T_CUSTOMER_INFO · tiền mặt 150tr = 200−20−30 · 4 cột tiền CỘNG KHỚP';
ELSE PRINT CONCAT('  !!! báo cáo cơ bản: ec=',@ecP2,' n=',@n1,' ck=',@ck1,' pct=',@pc1,' tkck=',ISNULL(@tk1,'(null)'),
                  ' lech_cong=',@nl1,' tienmat=',ISNULL(CONVERT(VARCHAR(20),@oh1),'(null)'),' cotuc=',ISNULL(CONVERT(VARCHAR(20),@dv1),'(null)'));
...
PRINT '';
PRINT '======== INGEST: tách khoản tiền (cổ tức chờ về / bán CK chờ về) ========';
-- Asset gửi `cash` là TỔNG ĐÃ GỘP; hai khoản chờ về là tách khoản của nó.
INSERT T_SI_PORTFOLIO (C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE,C_JOIN_DATE,C_STATUS)
VALUES ('PND1','PC1','SDI01','2026-01-02','ACTIVE');
DECLARE @ecPd INT, @emPd NVARCHAR(400), @rwPd BIGINT;

-- (a) payload CÓ 2 field → lưu đủ, tiền mặt suy ra = 100−20−30 = 50
EXEC SP_INGEST_ASSET_NAV
     N'[{"si_account":"PND1","aum":1000,"daily_return":0,"cash":100,"cash_available":50,"dividend_pending":20,"sell_pending":30}]',
     '2026-01-06', 'smoke', @ecPd OUTPUT, @emPd OUTPUT, @rwPd OUTPUT;
DECLARE @dv INT=(SELECT C_DIVIDEND_PENDING FROM T_SI_BALANCE WHERE C_SI_ACCOUNT='PND1' AND C_BUSINESS_DATE='2026-01-06');
DECLARE @sp INT=(SELECT C_SELL_PENDING     FROM T_SI_BALANCE WHERE C_SI_ACCOUNT='PND1' AND C_BUSINESS_DATE='2026-01-06');
DECLARE @dvC INT=(SELECT C_DIVIDEND_PENDING FROM T_SI_CURRENT WHERE C_SI_ACCOUNT='PND1');
IF @ecPd=0 AND @dv=20 AND @sp=30 AND @dvC=20
   PRINT '  OK ingest 2 khoản chờ về → lưu cả T_SI_BALANCE lẫn T_SI_CURRENT';
ELSE PRINT CONCAT('  !!! ingest tách khoản: ec=',@ecPd,' div=',@dv,' sell=',@sp,' div_current=',@dvC,' ',ISNULL(@emPd,''));

-- (b) payload CŨ (không có 2 field) → 0, KHÔNG vỡ ingest cũ
EXEC SP_INGEST_ASSET_NAV
     N'[{"si_account":"PND1","aum":1000,"daily_return":0,"cash":100,"cash_available":50}]',
     '2026-01-07', 'smoke', @ecPd OUTPUT, @emPd OUTPUT, @rwPd OUTPUT;
DECLARE @dv2 INT=(SELECT C_DIVIDEND_PENDING FROM T_SI_BALANCE WHERE C_SI_ACCOUNT='PND1' AND C_BUSINESS_DATE='2026-01-07');
IF @ecPd=0 AND @dv2=0 PRINT '  OK payload CŨ thiếu 2 field → 0, ingest không vỡ (backward-compat)';
ELSE PRINT CONCAT('  !!! backward-compat: ec=',@ecPd,' div=',@dv2);

-- (c) ★ div+sell > cash → CHẶN err=22 (nếu không, "Tiền mặt" trên báo cáo ra ÂM)
EXEC SP_INGEST_ASSET_NAV
     N'[{"si_account":"PND1","aum":1000,"daily_return":0,"cash":100,"cash_available":50,"dividend_pending":80,"sell_pending":50}]',
     '2026-01-08', 'smoke', @ecPd OUTPUT, @emPd OUTPUT, @rwPd OUTPUT;
DECLARE @nB INT=(SELECT COUNT(*) FROM T_SI_BALANCE WHERE C_SI_ACCOUNT='PND1' AND C_BUSINESS_DATE='2026-01-08');
IF @ecPd=22 AND @nB=0 PRINT '  OK div+sell > cash → err=22, KHÔNG ghi dòng nào (chặn tiền mặt ÂM)';
ELSE PRINT CONCAT('  !!! guard div+sell>cash: ec=',@ecPd,' #dong=',@nB);

DELETE FROM T_SI_CURRENT   WHERE C_SI_ACCOUNT='PND1';
DELETE FROM T_SI_BALANCE   WHERE C_SI_ACCOUNT='PND1';
DELETE FROM T_SI_PORTFOLIO WHERE C_SI_ACCOUNT='PND1';
