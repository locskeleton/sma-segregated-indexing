...
PRINT '';
PRINT '======== INDEX completeness IN-PROC: thiếu giá 1 mã rổ → SKIP master (không ghi index sai) ========';
INSERT T_MASTER_PORTFOLIO (C_MASTER_CODE,C_MASTER_NAME,C_STATUS,C_INCEPTION_DATE,C_BENCHMARK_CODE) VALUES ('MMISS',N'MissPx','ACTIVE','2026-05-01','VNINDEX');
INSERT T_MASTER_PORTFOLIO_TICKER (C_MASTER_CODE,C_EFFECTIVE_DATE,C_TICKER,C_TARGET_WEIGHT) VALUES ('MMISS','2026-05-01','PXM1',50),('MMISS','2026-05-01','PXM2',50);
INSERT T_PRICE_DAILY (C_TICKER,C_BUSINESS_DATE,C_REF_PRICE,C_CLOSE_PRICE) VALUES ('PXM1','2026-05-01',100,100);  -- CHỈ PXM1 (PXM2 thiếu)
EXEC SP_EOD_SI_INDEX '2026-05-01';
IF NOT EXISTS (SELECT 1 FROM T_MASTER_INDEX_DAILY WHERE C_MASTER_CODE='MMISS' AND C_BUSINESS_DATE='2026-05-01')
   PRINT '  OK thiếu giá PXM2 → MMISS KHÔNG ghi index @05-01 (skip, không drop ngầm tính sai)';
ELSE PRINT '  !!! MMISS vẫn tính index dù thiếu giá PXM2';
INSERT T_PRICE_DAILY (C_TICKER,C_BUSINESS_DATE,C_REF_PRICE,C_CLOSE_PRICE) VALUES ('PXM1','2026-05-04',100,110),('PXM2','2026-05-04',100,100);  -- đủ 2 mã
EXEC SP_EOD_SI_INDEX '2026-05-04';
DECLARE @mmidx DECIMAL(18,2)=(SELECT C_INDEX_VALUE FROM T_MASTER_INDEX_DAILY WHERE C_MASTER_CODE='MMISS' AND C_BUSINESS_DATE='2026-05-04');
IF @mmidx=1050.00 PRINT CONCAT('  OK đủ giá cả 2 mã → MMISS có index @05-04 = ',@mmidx);
ELSE PRINT CONCAT('  !!! MMISS index @05-04 sai: ',ISNULL(CAST(@mmidx AS VARCHAR(20)),'(không có)'));
...
DELETE FROM T_PRICE_DAILY WHERE C_BUSINESS_DATE IN ('2026-05-01','2026-05-04');
