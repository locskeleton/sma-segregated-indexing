...
DELETE FROM T_SI_FEE_INCOME; DELETE FROM T_EOD_RUN; DELETE FROM T_MASTER_NAV_CURRENT;
...
VALUES ('SDI01',N'Demo','ACTIVE','2026-01-02',0,'VNINDEX');   -- rate=0: smoke giữ NAV=gross (phí QL test riêng)
INSERT INTO T_SI_PORTFOLIO (C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE,C_JOIN_DATE,C_STATUS,C_MGMT_FEE_RATE)
VALUES ('SUB00001001','KH00001001','SDI01','2026-01-02','ACTIVE',0);  -- sub-account = SUB00001001
...
EXEC SP_EOD_SET_SOURCE_READY @p_business_date='2026-01-02', @p_source='MKT_DATA', @p_total_record=1, @p_err_code=@ec OUTPUT, @p_err_msg=@em OUTPUT;
...
EXEC SP_EOD_SET_SOURCE_READY @p_business_date='2026-01-05', @p_source='MKT_DATA', @p_total_record=1, @p_err_code=@ec OUTPUT, @p_err_msg=@em OUTPUT;
...
EXEC SP_EOD_SET_SOURCE_READY @p_business_date='2026-01-06', @p_source='MKT_DATA', @p_total_record=1, @p_err_code=@ec OUTPUT, @p_err_msg=@em OUTPUT;
...
EXEC SP_EOD_SET_SOURCE_READY @p_business_date='2026-01-07', @p_source='MKT_DATA', @p_total_record=1, @p_err_code=@ec OUTPUT, @p_err_msg=@em OUTPUT;
...
-- Cổ tức/phí lưu ký per-ngày KHÔNG còn ở master balance → xem chi tiết per-KH ở T_SI_FEE_INCOME (block dưới).
...
SELECT C_BUSINESS_DATE, C_TYPE, C_TICKER, C_AMOUNT, C_SOURCE_EVENT_ID FROM T_SI_FEE_INCOME
