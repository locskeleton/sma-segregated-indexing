...
-- Cô lập: index hard-fail là all-or-nothing trên MỌI master ACTIVE. SDI01 ACTIVE nhưng KHÔNG có giá @04-xx
--   → để khỏi THROW, tạm CLOSE SDI01 trong test này (khôi phục ACTIVE ở cleanup).
UPDATE T_MASTER_PORTFOLIO SET C_STATUS='CLOSED' WHERE C_MASTER_CODE='SDI01';
...
UPDATE T_MASTER_PORTFOLIO SET C_STATUS='ACTIVE' WHERE C_MASTER_CODE='SDI01';  -- khôi phục sau cô lập test MPCT
...
PRINT '';
PRINT '======== INDEX completeness HARD-FAIL: thiếu giá 1 mã rổ → THROW, KHÔNG ghi index master nào (all-or-nothing) ========';
-- Test trên 2026-01-06 (SDI01 đủ giá AAA/BBB, index=1078.80). Thêm MMISS thiếu giá PXM2 → cả ngày phải THROW.
DECLARE @sdiBefore DECIMAL(18,2)=(SELECT C_INDEX_VALUE FROM T_MASTER_INDEX_DAILY WHERE C_MASTER_CODE='SDI01' AND C_BUSINESS_DATE='2026-01-06');
INSERT T_MASTER_PORTFOLIO (C_MASTER_CODE,C_MASTER_NAME,C_STATUS,C_INCEPTION_DATE,C_BENCHMARK_CODE) VALUES ('MMISS',N'MissPx','ACTIVE','2026-01-06','VNINDEX');
INSERT T_MASTER_PORTFOLIO_TICKER (C_MASTER_CODE,C_EFFECTIVE_DATE,C_TICKER,C_TARGET_WEIGHT) VALUES ('MMISS','2026-01-06','PXM1',50),('MMISS','2026-01-06','PXM2',50);
INSERT T_PRICE_DAILY (C_TICKER,C_BUSINESS_DATE,C_REF_PRICE,C_CLOSE_PRICE) VALUES ('PXM1','2026-01-06',100,110);  -- CHỈ PXM1 (PXM2 thiếu)
DECLARE @ecMM INT=0; DECLARE @emMM NVARCHAR(2000);
BEGIN TRY EXEC SP_EOD_SI_INDEX '2026-01-06'; END TRY
BEGIN CATCH SET @ecMM=ERROR_NUMBER(); SET @emMM=ERROR_MESSAGE(); END CATCH
DECLARE @sdiAfter DECIMAL(18,2)=(SELECT C_INDEX_VALUE FROM T_MASTER_INDEX_DAILY WHERE C_MASTER_CODE='SDI01' AND C_BUSINESS_DATE='2026-01-06');
IF @ecMM=51011 AND @sdiAfter=@sdiBefore AND NOT EXISTS (SELECT 1 FROM T_MASTER_INDEX_DAILY WHERE C_MASTER_CODE='MMISS' AND C_BUSINESS_DATE='2026-01-06')
   PRINT CONCAT('  OK thiếu PXM2 → THROW 51011 (báo rõ), index ngày GIỮ NGUYÊN (SDI01=',@sdiAfter,' không đổi), MMISS không ghi');
ELSE PRINT CONCAT('  !!! hard-fail sai: ecMM=',@ecMM,' sdiBefore=',@sdiBefore,' sdiAfter=',@sdiAfter,' msg=',ISNULL(@emMM,'(không throw)'));
-- đủ giá cả 2 mã → tính thành công cả ngày (SDI01 giữ 1078.80, MMISS=1000×1.05=1050.00)
INSERT T_PRICE_DAILY (C_TICKER,C_BUSINESS_DATE,C_REF_PRICE,C_CLOSE_PRICE) VALUES ('PXM2','2026-01-06',100,100);
EXEC SP_EOD_SI_INDEX '2026-01-06';
DECLARE @mmidx DECIMAL(18,2)=(SELECT C_INDEX_VALUE FROM T_MASTER_INDEX_DAILY WHERE C_MASTER_CODE='MMISS' AND C_BUSINESS_DATE='2026-01-06');
DECLARE @sdiFinal DECIMAL(18,2)=(SELECT C_INDEX_VALUE FROM T_MASTER_INDEX_DAILY WHERE C_MASTER_CODE='SDI01' AND C_BUSINESS_DATE='2026-01-06');
IF @mmidx=1050.00 AND @sdiFinal=1078.80 PRINT CONCAT('  OK đủ giá → cả ngày tính: MMISS=',@mmidx,' (base 1000×1.05), SDI01 giữ ',@sdiFinal);
ELSE PRINT CONCAT('  !!! sau khi đủ giá sai: MMISS=',ISNULL(CAST(@mmidx AS VARCHAR(20)),'(không có)'),' SDI01=',@sdiFinal);
-- cleanup: trả MMISS + giá test, khôi phục index SDI01@06 nguyên trạng (DELETE+INSERT @06 ở trên đã giữ đúng)
DELETE FROM T_MASTER_INDEX_DAILY WHERE C_MASTER_CODE='MMISS';
DELETE FROM T_MASTER_PORTFOLIO_TICKER WHERE C_MASTER_CODE='MMISS';
DELETE FROM T_MASTER_PORTFOLIO WHERE C_MASTER_CODE='MMISS';
DELETE FROM T_PRICE_DAILY WHERE C_TICKER IN ('PXM1','PXM2');

PRINT '';
PRINT '======== INDEX weight INVALID: Σweight=0 → raise TRƯỚC insert (không vỡ NOT NULL) ========';
-- Cô lập SDI01 (CLOSE) vì all-or-nothing. MZW có rổ đủ giá (qua completeness) nhưng Σweight=0 → phải THROW 51012.
UPDATE T_MASTER_PORTFOLIO SET C_STATUS='CLOSED' WHERE C_MASTER_CODE='SDI01';
...
INSERT T_MASTER_PORTFOLIO_TICKER (C_MASTER_CODE,C_EFFECTIVE_DATE,C_TICKER,C_TARGET_WEIGHT) VALUES ('MZW','2026-03-01','ZWA',0),('MZW','2026-03-01','ZWB',0);  -- Σ=0
INSERT T_PRICE_DAILY (C_TICKER,C_BUSINESS_DATE,C_REF_PRICE,C_CLOSE_PRICE) VALUES ('ZWA','2026-03-02',100,100),('ZWB','2026-03-02',50,50);  -- đủ giá → qua completeness, vướng weight
DECLARE @ecZW INT=0; DECLARE @emZW NVARCHAR(2000);
BEGIN TRY EXEC SP_EOD_SI_INDEX '2026-03-02'; END TRY
BEGIN CATCH SET @ecZW=ERROR_NUMBER(); SET @emZW=ERROR_MESSAGE(); END CATCH
IF @ecZW=51012 AND NOT EXISTS (SELECT 1 FROM T_MASTER_INDEX_DAILY WHERE C_MASTER_CODE='MZW' AND C_BUSINESS_DATE='2026-03-02')
   PRINT '  OK Σweight=0 → THROW 51012 TRƯỚC insert (không chạm NOT NULL), MZW không ghi';
ELSE PRINT CONCAT('  !!! weight-invalid guard sai: ec=',@ecZW,' msg=',ISNULL(@emZW,'(không throw)'));
DELETE FROM T_MASTER_PORTFOLIO_TICKER WHERE C_MASTER_CODE='MZW';
DELETE FROM T_MASTER_PORTFOLIO WHERE C_MASTER_CODE='MZW';
DELETE FROM T_PRICE_DAILY WHERE C_TICKER IN ('ZWA','ZWB');
UPDATE T_MASTER_PORTFOLIO SET C_STATUS='ACTIVE' WHERE C_MASTER_CODE='SDI01';  -- khôi phục sau cô lập
