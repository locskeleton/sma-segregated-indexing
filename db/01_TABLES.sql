/*==============================================================================
  ...
  Naming convention:
    ...
    - Cột:   prefix C_ (thường), UPPERCASE. Cột ENTITY-ID dùng prefix vai trò:
             ...
             Áp cho: SI_ID (PK_SI_ID@master, FK_SI_ID nơi khác), CUST_CODE (FK_CUST_CODE varchar10).
             Cột khác (business_date, ticker, benchmark_code, event_id…) giữ C_.
...
CREATE TABLE T_INDEXING_PORTFOLIO (
    FK_CUST_CODE     VARCHAR(10)     NOT NULL,
    FK_SI_ID          BIGINT          NOT NULL,
    ...
    CONSTRAINT PK_INDEXING_PORTFOLIO PRIMARY KEY (FK_CUST_CODE, FK_SI_ID)
...
CREATE TABLE T_CUSTOMER_HOLDING_DAILY (
    ...
    CONSTRAINT PK_CUSTOMER_HOLDING_DAILY PRIMARY KEY (C_BUSINESS_DATE, FK_CUST_CODE, FK_SI_ID, C_TICKER)
...
CREATE TABLE T_FO_CASH_SYNC (
    ...
    CONSTRAINT PK_FO_CASH_SYNC PRIMARY KEY (C_BUSINESS_DATE, FK_CUST_CODE, FK_SI_ID)
);
...
CREATE INDEX IX_CASHFLOW_EVENT_DATE ON T_CASHFLOW_EVENT (C_BUSINESS_DATE) INCLUDE (FK_CUST_CODE, FK_SI_ID, C_EVENT_TYPE, C_AMOUNT);
...
CREATE TABLE T_UNIT_LEDGER (
    ...
    CONSTRAINT PK_UNIT_LEDGER PRIMARY KEY (FK_CUST_CODE, FK_SI_ID, C_BUSINESS_DATE)
);
...
CREATE TABLE T_CUSTOMER_NAV_CURRENT (
    ...
    CONSTRAINT PK_CUSTOMER_NAV_CURRENT PRIMARY KEY (FK_CUST_CODE, FK_SI_ID)
) WITH (DATA_COMPRESSION = PAGE);
...
CREATE TABLE T_INDEXING_PORTFOLIO_TICKER (
    ...
    C_QUANTITY       DECIMAL(20,4)   NOT NULL,
    C_AVG_COST       DECIMAL(18,4)   NULL,
    CONSTRAINT PK_INDEXING_PORTFOLIO_TICKER PRIMARY KEY (FK_CUST_CODE, FK_SI_ID, C_TICKER)
) WITH (DATA_COMPRESSION = PAGE);
-- Prod: CREATE NONCLUSTERED COLUMNSTORE INDEX NCCI_INDEXING_PORTFOLIO_TICKER
--       ON T_INDEXING_PORTFOLIO_TICKER (FK_CUST_CODE, FK_SI_ID, C_TICKER, C_QUANTITY);
...
CREATE TABLE T_EOD_WORK (
    ...
    CONSTRAINT PK_EOD_WORK PRIMARY KEY (C_BUSINESS_DATE, FK_CUST_CODE, FK_SI_ID)
);
...
CREATE TABLE T_CUSTOMER_NAV_DAILY (
    ...
    CONSTRAINT PK_CUSTOMER_NAV_DAILY PRIMARY KEY (C_BUSINESS_DATE, FK_CUST_CODE, FK_SI_ID)
);
...
CREATE INDEX IX_CUSTOMER_FEE_INCOME_DATE ON T_CUSTOMER_FEE_INCOME (C_BUSINESS_DATE)
    INCLUDE (FK_CUST_CODE, FK_SI_ID, C_TYPE, C_AMOUNT);
