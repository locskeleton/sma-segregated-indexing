...
CREATE TABLE T_STRATEGY (
    ...
    C_STATUS         VARCHAR(10)     NOT NULL CONSTRAINT DF_STRATEGY_STATUS DEFAULT 'ACTIVE', -- ACTIVE|CLOSED
    C_INCEPTION_DATE DATE            NULL,
    C_MGMT_FEE_RATE  DECIMAL(9,6)    NOT NULL CONSTRAINT DF_STRATEGY_FEE DEFAULT 0.01,        -- %/năm
    C_BENCHMARK_ID   BIGINT          NULL,
    CONSTRAINT PK_STRATEGY PRIMARY KEY (C_SI_ID)
);

-- Danh mục mẫu (FO tính & feed). Σ C_TARGET_WEIGHT theo (C_SI_ID, C_EFFECTIVE_DATE) = 1.0 (100% cổ phiếu)
CREATE TABLE T_MODEL_WEIGHT (
    C_SI_ID          BIGINT          NOT NULL,
    C_EFFECTIVE_DATE DATE            NOT NULL,
    C_TICKER         VARCHAR(20)     NOT NULL,
    C_TARGET_WEIGHT  DECIMAL(12,8)   NOT NULL,
    CONSTRAINT PK_MODEL_WEIGHT PRIMARY KEY (C_SI_ID, C_EFFECTIVE_DATE, C_TICKER)
    -- FK_MODEL_WEIGHT__STRATEGY: C_SI_ID -> T_STRATEGY.C_SI_ID
);

-- Cấu hình đầu tư KH (1 dòng = 1 tiểu khoản = KH x SI)
CREATE TABLE T_CUSTOMER_SI (
    C_CUSTOMER_ID    BIGINT          NOT NULL,
    C_SI_ID          BIGINT          NOT NULL,
    C_SUB_ACCOUNT_NO VARCHAR(32)     NULL,
    C_JOIN_DATE      DATE            NOT NULL,
    C_STATUS         VARCHAR(10)     NOT NULL CONSTRAINT DF_CUSTSI_STATUS DEFAULT 'ACTIVE',
    C_INITIAL_AMOUNT DECIMAL(20,4)   NULL,
    C_SIP_AMOUNT     DECIMAL(20,4)   NULL,
    C_SIP_SCHEDULE   VARCHAR(50)     NULL,
    C_MGMT_FEE_RATE  DECIMAL(9,6)    NULL,    -- override; NULL = lấy theo T_STRATEGY
    C_MIN_INVEST     DECIMAL(20,4)   NULL,
    CONSTRAINT PK_CUSTOMER_SI PRIMARY KEY (C_CUSTOMER_ID, C_SI_ID)
    -- FK_CUSTOMER_SI__STRATEGY: C_SI_ID -> T_STRATEGY.C_SI_ID
);
