...
/*==============================================================================
  ...
  Naming convention:
    ...
    - Cột:   prefix C_ (thường), UPPERCASE. Khóa nghiệp vụ giữ C_:
             C_SI_CODE (mã SI — PK của T_MASTER_PORTFOLIO, các bảng khác FK theo C_SI_CODE),
             C_CUST_CODE varchar10, C_TICKER, C_BENCHMARK_CODE, C_BUSINESS_DATE…
    - Khóa surrogate public (GUID, API/UI, IDOR-safe): prefix PK_<table> ở bảng GỐC
             (vd PK_MASTER_PORTFOLIO_TICKER) | FK_<table> khi tham chiếu ở bảng khác.
             (T_MASTER_PORTFOLIO KHÔNG có surrogate — C_SI_CODE là khóa public luôn.)
    - PK constraint: prefix PK_<table> (natural composite + có surrogate → _NK)
    - FK: KHÔNG hard-set constraint; đánh dấu qua tên cột
...
/*------------------------------------------------------------------ MASTER ---*/
-- C_SI_CODE = PK + khóa public (mã nghiệp vụ, không phải int tuần tự → an toàn IDOR).
...
CREATE TABLE T_MASTER_PORTFOLIO_TICKER (
    PK_MASTER_PORTFOLIO_TICKER UNIQUEIDENTIFIER NOT NULL CONSTRAINT DF_MASTER_PORTFOLIO_TICKER_PKID DEFAULT NEWID(), -- khóa public API/UI
    C_SI_CODE        VARCHAR(20)     NOT NULL,
    C_EFFECTIVE_DATE DATE            NOT NULL,
    C_TICKER         VARCHAR(20)     NOT NULL,
    C_TARGET_WEIGHT  DECIMAL(12,8)   NOT NULL,
    CONSTRAINT PK_MASTER_PORTFOLIO_TICKER_NK PRIMARY KEY (C_SI_CODE, C_EFFECTIVE_DATE, C_TICKER),
    CONSTRAINT UQ_MASTER_PORTFOLIO_TICKER_PKID UNIQUE NONCLUSTERED (PK_MASTER_PORTFOLIO_TICKER)
    -- FK_MASTER_PORTFOLIO_TICKER (ở bảng khác) -> PK_MASTER_PORTFOLIO_TICKER; C_SI_CODE -> T_MASTER_PORTFOLIO.C_SI_CODE
);

-- Cấu hình đầu tư KH (1 dòng = 1 tiểu khoản = KH x SI)
CREATE TABLE T_INDEXING_PORTFOLIO (
    PK_INDEXING_PORTFOLIO UNIQUEIDENTIFIER NOT NULL CONSTRAINT DF_INDEXING_PORTFOLIO_PKID DEFAULT NEWID(), -- khóa public API/UI (IDOR-safe)
    ...
    CONSTRAINT PK_INDEXING_PORTFOLIO_NK PRIMARY KEY (C_CUST_CODE, C_SI_CODE),
    CONSTRAINT UQ_INDEXING_PORTFOLIO_PKID UNIQUE NONCLUSTERED (PK_INDEXING_PORTFOLIO)
...
CREATE TABLE T_REBALANCE_REQUEST (
    PK_REBALANCE_REQUEST UNIQUEIDENTIFIER NOT NULL CONSTRAINT DF_REBREQ_PKID DEFAULT NEWID(), -- khóa public API/UI
    ...
    CONSTRAINT PK_REBALANCE_REQUEST_NK PRIMARY KEY (C_REQUEST_ID),
    CONSTRAINT UQ_REBALANCE_REQUEST_PKID UNIQUE NONCLUSTERED (PK_REBALANCE_REQUEST)
    -- C_SI_CODE -> T_MASTER_PORTFOLIO.C_SI_CODE
);
...
CREATE TABLE T_CUSTOMER_HOLDING_HIST (
    ...
    CONSTRAINT PK_CUSTOMER_HOLDING_HIST PRIMARY KEY (C_CUST_CODE, C_SI_CODE, C_TICKER, C_VALID_FROM)
...
CREATE TABLE T_FO_CASH_SYNC (
    ...
    CONSTRAINT PK_FO_CASH_SYNC PRIMARY KEY (C_BUSINESS_DATE, C_CUST_CODE, C_SI_CODE)
);
...
CREATE TABLE T_CUSTOMER_CASH_HIST (
    ...
    CONSTRAINT PK_CUSTOMER_CASH_HIST PRIMARY KEY (C_CUST_CODE, C_SI_CODE, C_VALID_FROM)
) WITH (DATA_COMPRESSION = PAGE);
...
CREATE TABLE T_UNIT_LEDGER (
    ...
    CONSTRAINT PK_UNIT_LEDGER PRIMARY KEY (C_CUST_CODE, C_SI_CODE, C_BUSINESS_DATE)
);
...
CREATE TABLE T_CUSTOMER_NAV_CURRENT (
    ...
    CONSTRAINT PK_CUSTOMER_NAV_CURRENT PRIMARY KEY (C_CUST_CODE, C_SI_CODE)
) WITH (DATA_COMPRESSION = PAGE);
...
CREATE TABLE T_INDEXING_PORTFOLIO_TICKER (
    ...
    C_QUANTITY       DECIMAL(20,4)   NOT NULL,
    C_AVG_COST       DECIMAL(18,4)   NULL,
    CONSTRAINT PK_INDEXING_PORTFOLIO_TICKER PRIMARY KEY (C_CUST_CODE, C_SI_CODE, C_TICKER)
) WITH (DATA_COMPRESSION = PAGE);
...
CREATE TABLE T_EOD_WORK (
    ...
    CONSTRAINT PK_EOD_WORK PRIMARY KEY (C_BUSINESS_DATE, C_CUST_CODE, C_SI_CODE)
);
...
CREATE TABLE T_CUSTOMER_NAV_DAILY (
    ...
    CONSTRAINT PK_CUSTOMER_NAV_DAILY PRIMARY KEY (C_BUSINESS_DATE, C_CUST_CODE, C_SI_CODE)
);
...
CREATE TABLE T_SI_INDEX_DAILY (
    ...
    C_DAILY_RETURN   DECIMAL(18,10)  NULL,
    CONSTRAINT PK_SI_INDEX_DAILY PRIMARY KEY (C_BUSINESS_DATE, C_SI_CODE)
);
...
CREATE TABLE T_SI_HOLDING_DAILY (
    ...
    CONSTRAINT PK_SI_HOLDING_DAILY PRIMARY KEY (C_BUSINESS_DATE, C_SI_CODE, C_TICKER)
);
...
CREATE TABLE T_SI_NAV_DAILY (
    ...
    C_DAILY_PNL        DECIMAL(20,4)  NOT NULL,
    C_DAILY_RETURN     DECIMAL(18,10) NULL,
    CONSTRAINT PK_SI_NAV_DAILY PRIMARY KEY (C_BUSINESS_DATE, C_SI_CODE)
);
...
CREATE TABLE T_SI_NAV_CURRENT (
    ...
    C_LAST_UNIT_PRICE  DECIMAL(28,10) NULL,
    C_LAST_BUSINESS_DATE DATE         NULL,
    CONSTRAINT PK_SI_NAV_CURRENT PRIMARY KEY (C_SI_CODE)
);
