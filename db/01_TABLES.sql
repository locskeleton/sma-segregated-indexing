...
CREATE TABLE T_INDEXING_PORTFOLIO (
    ...
    C_STATUS         VARCHAR(10)     NOT NULL CONSTRAINT DF_INDEXING_PORTFOLIO_STATUS DEFAULT 'ACTIVE', -- ACTIVE|CLOSED
    ...
    CONSTRAINT UQ_INDEXING_PORTFOLIO_NK UNIQUE (C_SI_ACCOUNT)   -- mã sub-account duy nhất toàn cục
);
-- Tối đa 1 sub-account ACTIVE / (KH × master) tại 1 thời điểm (closed thì được mở lại = dòng mới)
CREATE UNIQUE INDEX UQ_INDEXING_PORTFOLIO_ACTIVE ON T_INDEXING_PORTFOLIO (C_CUST_CODE, C_MASTER_CODE)
...
CREATE TABLE T_CUSTOMER_HOLDING_HIST (
    ...
    PK_CUSTOMER_HOLDING_HIST UNIQUEIDENTIFIER NOT NULL CONSTRAINT DF_CUSTOMER_HOLDING_HIST_PKID DEFAULT NEWID(),
    ...
    CONSTRAINT UQ_CUSTOMER_HOLDING_HIST_PKID UNIQUE NONCLUSTERED (PK_CUSTOMER_HOLDING_HIST),
    CONSTRAINT UQ_CUSTOMER_HOLDING_HIST_NK UNIQUE (C_SI_ACCOUNT, C_TICKER, C_VALID_FROM)
) WITH (DATA_COMPRESSION = PAGE);
CREATE INDEX IX_CUSTOMER_HOLDING_HIST_OPEN ON T_CUSTOMER_HOLDING_HIST (C_SI_ACCOUNT, C_TICKER)
    INCLUDE (C_QUANTITY, C_AVG_COST) WHERE C_VALID_TO IS NULL;
-- (Cash vào qua SP_INGEST_CUSTOMER → state.C_CASH + diff T_CUSTOMER_CASH_HIST; không có bảng feed batch.)
...
CREATE TABLE T_CUSTOMER_CASH_HIST (
    ...
    PK_CUSTOMER_CASH_HIST UNIQUEIDENTIFIER NOT NULL CONSTRAINT DF_CUSTOMER_CASH_HIST_PKID DEFAULT NEWID(),
    ...
    CONSTRAINT UQ_CUSTOMER_CASH_HIST_PKID UNIQUE NONCLUSTERED (PK_CUSTOMER_CASH_HIST),
    CONSTRAINT UQ_CUSTOMER_CASH_HIST_NK UNIQUE (C_SI_ACCOUNT, C_VALID_FROM)
) WITH (DATA_COMPRESSION = PAGE);
CREATE INDEX IX_CUSTOMER_CASH_HIST_OPEN ON T_CUSTOMER_CASH_HIST (C_SI_ACCOUNT)
...
CREATE TABLE T_CASHFLOW_EVENT (
    ...
    CONSTRAINT PK_CASHFLOW_EVENT_ID PRIMARY KEY CLUSTERED (C_EVENT_ID),
    CONSTRAINT UQ_CASHFLOW_EVENT_PKID UNIQUE NONCLUSTERED (PK_CASHFLOW_EVENT)
);
CREATE INDEX IX_CASHFLOW_EVENT_DATE ON T_CASHFLOW_EVENT (C_BUSINESS_DATE) INCLUDE (C_SI_ACCOUNT, C_EVENT_TYPE, C_AMOUNT);

-- Unit thay đổi (ghi dòng khi có cashflow) per sub-account
CREATE TABLE T_UNIT_LEDGER (
    C_UNIT_LEDGER_ID BIGINT IDENTITY(1,1) NOT NULL,
    PK_UNIT_LEDGER   UNIQUEIDENTIFIER NOT NULL CONSTRAINT DF_UNIT_LEDGER_PKID DEFAULT NEWID(),
    C_SI_ACCOUNT     VARCHAR(20)     NOT NULL,
    C_CUST_CODE      VARCHAR(10)     NOT NULL,
    C_MASTER_CODE    VARCHAR(20)     NOT NULL,
    C_BUSINESS_DATE  DATE            NOT NULL,
    ...
    CONSTRAINT UQ_UNIT_LEDGER_PKID UNIQUE NONCLUSTERED (PK_UNIT_LEDGER),
    CONSTRAINT UQ_UNIT_LEDGER_NK UNIQUE (C_SI_ACCOUNT, C_BUSINESS_DATE)
);
...
CREATE TABLE T_CUSTOMER_NAV_CURRENT (
    ...
    CONSTRAINT PK_CUSTOMER_NAV_CURRENT_NK PRIMARY KEY CLUSTERED (C_SI_ACCOUNT),  -- natural clustered (MERGE/point roll-forward)
    CONSTRAINT UQ_CUSTOMER_NAV_CURRENT_PKID UNIQUE NONCLUSTERED (PK_CUSTOMER_NAV_CURRENT)
) WITH (DATA_COMPRESSION = PAGE);
...
CREATE TABLE T_INDEXING_PORTFOLIO_TICKER (
    ...
    C_QUANTITY       DECIMAL(20,0)   NOT NULL,
    C_AVG_COST       DECIMAL(18,4)   NULL,
    CONSTRAINT PK_INDEXING_PORTFOLIO_TICKER_NK PRIMARY KEY CLUSTERED (C_SI_ACCOUNT, C_TICKER),  -- natural clustered (MTM)
    CONSTRAINT UQ_INDEXING_PORTFOLIO_TICKER_PKID UNIQUE NONCLUSTERED (PK_INDEXING_PORTFOLIO_TICKER)
) WITH (DATA_COMPRESSION = PAGE);
...
CREATE TABLE T_CUSTOMER_NAV_BALANCE (
    ...
    PK_CUSTOMER_NAV_BALANCE UNIQUEIDENTIFIER NOT NULL CONSTRAINT DF_CUSTOMER_NAV_BALANCE_PKID DEFAULT NEWID(),
    C_BUSINESS_DATE  DATE            NOT NULL,
    C_SI_ACCOUNT     VARCHAR(20)     NOT NULL,
    C_CUST_CODE      VARCHAR(10)     NOT NULL,
    C_MASTER_CODE    VARCHAR(20)     NOT NULL,
    ...
    CONSTRAINT UQ_CUSTOMER_NAV_BALANCE_PKID UNIQUE NONCLUSTERED (PK_CUSTOMER_NAV_BALANCE),
    CONSTRAINT UQ_CUSTOMER_NAV_BALANCE_NK UNIQUE (C_BUSINESS_DATE, C_SI_ACCOUNT) -- idempotency
);
