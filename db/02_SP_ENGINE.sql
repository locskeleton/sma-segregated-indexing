...
BEGIN
    ...
    INSERT INTO T_MASTER_NAV_BALANCE (C_BUSINESS_DATE,C_MASTER_CODE,C_CASH,C_PENDING_CASH,C_DIV_CASH,
                             C_PAYABLE_FEE,C_AUM,
                             C_NAV,C_UNIT,C_UNIT_PRICE,C_DAILY_PNL,C_DAILY_RETURN,C_CASH_IN,C_CASH_OUT,C_TOTAL_ACCOUNT)
    SELECT @p_d, a.C_MASTER_CODE, a.CASH, a.PEND, a.DIVC,
           a.PAY, (a.CASH + a.PEND + a.DIVC + a.STOCK),   -- [BRD] AUM gross gồm stock (từ work) + receivables; KHÔNG lưu cột stock riêng (PM không đọc)
           a.NAV, a.UNT,
    ...
    USING (SELECT C_MASTER_CODE,C_CASH,C_PENDING_CASH,C_DIV_CASH,C_AUM,C_NAV,C_UNIT,C_UNIT_PRICE,C_TOTAL_ACCOUNT,C_BUSINESS_DATE
           FROM T_MASTER_NAV_BALANCE WHERE C_BUSINESS_DATE=@p_d) s
    ON t.C_MASTER_CODE=s.C_MASTER_CODE
    WHEN MATCHED THEN UPDATE SET
        t.C_CASH=s.C_CASH, t.C_PENDING_CASH=s.C_PENDING_CASH, t.C_DIV_CASH=s.C_DIV_CASH, t.C_AUM=s.C_AUM,
    ...
    WHEN NOT MATCHED THEN INSERT (C_MASTER_CODE,C_CASH,C_PENDING_CASH,C_DIV_CASH,C_AUM,C_LAST_NAV,C_UNIT,C_LAST_UNIT_PRICE,C_TOTAL_ACCOUNT,C_LAST_BUSINESS_DATE)
        VALUES (s.C_MASTER_CODE,s.C_CASH,s.C_PENDING_CASH,s.C_DIV_CASH,s.C_AUM,s.C_NAV,s.C_UNIT,s.C_UNIT_PRICE,s.C_TOTAL_ACCOUNT,s.C_BUSINESS_DATE);
END
GO
