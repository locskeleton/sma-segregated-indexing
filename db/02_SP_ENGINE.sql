...
BEGIN
    ...
    BEGIN TRY
        ...
        DECLARE @sub TABLE (C_MASTER_CODE VARCHAR(20) PRIMARY KEY, C_CASH DECIMAL(20,0));
        INSERT INTO @sub (C_MASTER_CODE, C_CASH)
        SELECT C_MASTER_CODE, C_CASH FROM OPENJSON(@json,'$.sub_accounts')
        WITH (C_MASTER_CODE VARCHAR(20) '$.si_code', C_CASH DECIMAL(20,0) '$.cash');

        -- FORWARD guard: tiểu khoản đã sync ngày MỚI HƠN @d ⇒ event quá khứ
        IF EXISTS (SELECT 1 FROM @sub n JOIN T_CUSTOMER_NAV_CURRENT s
                   ON s.C_CUST_CODE=@cust AND s.C_MASTER_CODE=n.C_MASTER_CODE WHERE s.C_LAST_SYNC_DATE > @d)
        ...
        DECLARE @hold TABLE (C_MASTER_CODE VARCHAR(20), C_TICKER VARCHAR(20), C_QUANTITY DECIMAL(20,0), C_AVG_COST DECIMAL(18,4));
        INSERT INTO @hold
        SELECT sa.C_MASTER_CODE, h.C_TICKER, h.C_QUANTITY, h.C_AVG_COST
        FROM OPENJSON(@json,'$.sub_accounts') WITH (C_MASTER_CODE VARCHAR(20) '$.si_code', holdings NVARCHAR(MAX) '$.holdings' AS JSON) sa
        ...
        WHERE t.C_CUST_CODE=@cust AND t.C_MASTER_CODE IN (SELECT C_MASTER_CODE FROM @sub);
        INSERT INTO T_INDEXING_PORTFOLIO_TICKER (C_CUST_CODE,C_MASTER_CODE,C_TICKER,C_QUANTITY,C_AVG_COST)
        SELECT @cust, C_MASTER_CODE, C_TICKER, C_QUANTITY, C_AVG_COST FROM @hold;

        UPDATE h SET C_VALID_TO=@d
        FROM T_CUSTOMER_HOLDING_HIST h
        LEFT JOIN T_INDEXING_PORTFOLIO_TICKER c
          ON c.C_CUST_CODE=h.C_CUST_CODE AND c.C_MASTER_CODE=h.C_MASTER_CODE AND c.C_TICKER=h.C_TICKER
        WHERE h.C_CUST_CODE=@cust AND h.C_MASTER_CODE IN (SELECT C_MASTER_CODE FROM @sub) AND h.C_VALID_TO IS NULL
          AND ( c.C_CUST_CODE IS NULL OR c.C_QUANTITY<>h.C_QUANTITY OR ISNULL(c.C_AVG_COST,-1)<>ISNULL(h.C_AVG_COST,-1) );
        INSERT INTO T_CUSTOMER_HOLDING_HIST (C_CUST_CODE,C_MASTER_CODE,C_TICKER,C_VALID_FROM,C_VALID_TO,C_QUANTITY,C_AVG_COST)
        SELECT c.C_CUST_CODE,c.C_MASTER_CODE,c.C_TICKER,@d,NULL,c.C_QUANTITY,c.C_AVG_COST
        FROM T_INDEXING_PORTFOLIO_TICKER c
        WHERE c.C_CUST_CODE=@cust AND c.C_MASTER_CODE IN (SELECT C_MASTER_CODE FROM @sub)
          AND NOT EXISTS (SELECT 1 FROM T_CUSTOMER_HOLDING_HIST h
              WHERE h.C_VALID_TO IS NULL AND h.C_CUST_CODE=c.C_CUST_CODE AND h.C_MASTER_CODE=c.C_MASTER_CODE AND h.C_TICKER=c.C_TICKER
        ...
        UPDATE h SET C_VALID_TO=@d
        FROM T_CUSTOMER_CASH_HIST h JOIN @sub n ON n.C_MASTER_CODE=h.C_MASTER_CODE
        WHERE h.C_CUST_CODE=@cust AND h.C_VALID_TO IS NULL AND h.C_CASH<>n.C_CASH;
        INSERT INTO T_CUSTOMER_CASH_HIST (C_CUST_CODE,C_MASTER_CODE,C_VALID_FROM,C_VALID_TO,C_CASH)
        SELECT @cust, n.C_MASTER_CODE, @d, NULL, n.C_CASH FROM @sub n
        WHERE NOT EXISTS (SELECT 1 FROM T_CUSTOMER_CASH_HIST h
            WHERE h.C_VALID_TO IS NULL AND h.C_CUST_CODE=@cust AND h.C_MASTER_CODE=n.C_MASTER_CODE AND h.C_CASH=n.C_CASH);

        MERGE T_CUSTOMER_NAV_CURRENT s
        USING @sub n ON s.C_CUST_CODE=@cust AND s.C_MASTER_CODE=n.C_MASTER_CODE
        WHEN MATCHED THEN UPDATE SET s.C_CASH=n.C_CASH, s.C_LAST_SYNC_DATE=@d
        WHEN NOT MATCHED THEN INSERT (C_CUST_CODE,C_MASTER_CODE,C_UNIT,C_CASH,C_PAYABLE_FEE,C_LAST_NAV,C_LAST_UNIT_PRICE,C_STATUS,C_LAST_SYNC_DATE)
            VALUES (@cust,n.C_MASTER_CODE,0,n.C_CASH,0,0,NULL,'ACTIVE',@d);

        /* CỔ TỨC/PHÍ: append, dedup theo C_SOURCE_EVENT_ID (idempotent redelivery) */
        INSERT INTO T_CUSTOMER_FEE_INCOME (C_BUSINESS_DATE,C_CUST_CODE,C_MASTER_CODE,C_TYPE,C_TICKER,C_AMOUNT,C_SOURCE,C_SOURCE_EVENT_ID)
        SELECT @d, @cust, f.C_MASTER_CODE, f.C_TYPE, f.C_TICKER, f.C_AMOUNT, 'FO', f.event_id
        FROM (
            SELECT sa.C_MASTER_CODE, x.event_id, x.C_TYPE, x.C_TICKER, x.C_AMOUNT
            FROM OPENJSON(@json,'$.sub_accounts') WITH (C_MASTER_CODE VARCHAR(20) '$.si_code', fees NVARCHAR(MAX) '$.fees' AS JSON) sa
...
BEGIN
    ...
    JOIN T_CUSTOMER_NAV_CURRENT s ON s.C_CUST_CODE=h.C_CUST_CODE AND s.C_MASTER_CODE=h.C_MASTER_CODE
    WHERE h.C_VALID_TO IS NULL AND s.C_CASH <> h.C_CASH;
    INSERT INTO T_CUSTOMER_CASH_HIST (C_CUST_CODE,C_MASTER_CODE,C_VALID_FROM,C_VALID_TO,C_CASH)
    SELECT s.C_CUST_CODE,s.C_MASTER_CODE,@d,NULL,s.C_CASH
    FROM T_CUSTOMER_NAV_CURRENT s
    WHERE NOT EXISTS (SELECT 1 FROM T_CUSTOMER_CASH_HIST h
        WHERE h.C_VALID_TO IS NULL AND h.C_CUST_CODE=s.C_CUST_CODE AND h.C_MASTER_CODE=s.C_MASTER_CODE AND h.C_CASH=s.C_CASH);
END
GO
...
BEGIN
    ...
    INSERT INTO T_EOD_WORK (C_BUSINESS_DATE,C_CUST_CODE,C_MASTER_CODE,C_CASH,C_PAYABLE_FEE,C_LAST_NAV,C_LAST_UNIT_PRICE,C_UNIT_PREV)
    SELECT @d,C_CUST_CODE,C_MASTER_CODE,C_CASH,C_PAYABLE_FEE,C_LAST_NAV,C_LAST_UNIT_PRICE,C_UNIT
    ...
    FROM T_EOD_WORK w
    JOIN (
        SELECT C_CUST_CODE,C_MASTER_CODE,
        ...
        FROM T_CASHFLOW_EVENT WHERE C_BUSINESS_DATE=@d GROUP BY C_CUST_CODE,C_MASTER_CODE
    ) cf ON cf.C_CUST_CODE=w.C_CUST_CODE AND cf.C_MASTER_CODE=w.C_MASTER_CODE
    ...
    FROM T_EOD_WORK w
    JOIN (
        SELECT h.C_CUST_CODE, h.C_MASTER_CODE, SUM(h.C_QUANTITY * p.C_CLOSE_PRICE) AS SV
        FROM T_INDEXING_PORTFOLIO_TICKER h
        JOIN T_PRICE_DAILY p ON p.C_TICKER=h.C_TICKER AND p.C_BUSINESS_DATE=@d
        GROUP BY h.C_CUST_CODE, h.C_MASTER_CODE
    ) m ON m.C_CUST_CODE=w.C_CUST_CODE AND m.C_MASTER_CODE=w.C_MASTER_CODE
    WHERE w.C_BUSINESS_DATE=@d;
    ...
    FROM T_CUSTOMER_NAV_CURRENT s
    JOIN T_EOD_WORK w ON w.C_CUST_CODE=s.C_CUST_CODE AND w.C_MASTER_CODE=s.C_MASTER_CODE AND w.C_BUSINESS_DATE=@d;

    -- unit ledger (chỉ ngày có cashflow)
    DELETE FROM T_UNIT_LEDGER WHERE C_BUSINESS_DATE=@d;
    INSERT INTO T_UNIT_LEDGER (C_CUST_CODE,C_MASTER_CODE,C_BUSINESS_DATE,C_CF_NET,C_DELTA_UNIT,C_UNIT)
    SELECT C_CUST_CODE,C_MASTER_CODE,@d,(C_CF_IN-C_CF_OUT),C_DELTA_UNIT,C_UNIT
    ...
    INSERT INTO T_CUSTOMER_NAV_BALANCE (C_BUSINESS_DATE,C_CUST_CODE,C_MASTER_CODE,C_NAV,C_UNIT,C_UNIT_PRICE,C_DAILY_PNL,C_DAILY_RETURN)
    SELECT @d, C_CUST_CODE, C_MASTER_CODE, C_NAV, C_UNIT, C_UNIT_PRICE, C_DAILY_PNL,
...
BEGIN
    ...
    fee AS (   -- cổ tức + phí per-KH → SUM lên SI (sparse). SI không có sự kiện nào → NULL
        ...
        FROM T_CUSTOMER_FEE_INCOME WHERE C_BUSINESS_DATE=@d GROUP BY C_MASTER_CODE
    )
...
BEGIN
    ...
    ;WITH sih AS (
        ...
        FROM T_INDEXING_PORTFOLIO_TICKER h GROUP BY h.C_MASTER_CODE, h.C_TICKER
    ),
    val AS (
