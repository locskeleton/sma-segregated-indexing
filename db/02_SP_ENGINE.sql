...
BEGIN
    ...
    INSERT INTO T_INDEXING_PORTFOLIO_TICKER (FK_CUST_CODE,FK_SI_ID,C_TICKER,C_QUANTITY,C_AVG_COST)
    SELECT FK_CUST_CODE,FK_SI_ID,C_TICKER,C_QUANTITY,C_AVG_COST
    ...
    USING (SELECT FK_CUST_CODE,FK_SI_ID,C_CASH FROM T_FO_CASH_SYNC WHERE C_BUSINESS_DATE=@d) f
    ON s.FK_CUST_CODE=f.FK_CUST_CODE AND s.FK_SI_ID=f.FK_SI_ID
    WHEN MATCHED THEN UPDATE SET C_CASH = f.C_CASH
    WHEN NOT MATCHED THEN INSERT (FK_CUST_CODE,FK_SI_ID,C_UNIT,C_CASH,C_PAYABLE_FEE,C_LAST_NAV,C_LAST_UNIT_PRICE,C_STATUS)
        VALUES (f.FK_CUST_CODE,f.FK_SI_ID,0,f.C_CASH,0,0,NULL,'ACTIVE');
END
GO
...
BEGIN
    ...
    INSERT INTO T_EOD_WORK (C_BUSINESS_DATE,FK_CUST_CODE,FK_SI_ID,C_CASH,C_PAYABLE_FEE,C_LAST_NAV,C_LAST_UNIT_PRICE,C_UNIT_PREV)
    SELECT @d,FK_CUST_CODE,FK_SI_ID,C_CASH,C_PAYABLE_FEE,C_LAST_NAV,C_LAST_UNIT_PRICE,C_UNIT
    ...
    JOIN (
        ...
        FROM T_CASHFLOW_EVENT WHERE C_BUSINESS_DATE=@d GROUP BY FK_CUST_CODE,FK_SI_ID
    ) cf ON cf.FK_CUST_CODE=w.FK_CUST_CODE AND cf.FK_SI_ID=w.FK_SI_ID
    ...
    FROM T_EOD_WORK w
    JOIN (
        SELECT h.FK_CUST_CODE, h.FK_SI_ID, SUM(h.C_QUANTITY * p.C_CLOSE_PRICE) AS SV
        FROM T_INDEXING_PORTFOLIO_TICKER h
        JOIN T_PRICE_DAILY p ON p.C_TICKER=h.C_TICKER AND p.C_BUSINESS_DATE=@d
        GROUP BY h.FK_CUST_CODE, h.FK_SI_ID
    ) m ON m.FK_CUST_CODE=w.FK_CUST_CODE AND m.FK_SI_ID=w.FK_SI_ID
    WHERE w.C_BUSINESS_DATE=@d;
    ...
    JOIN T_EOD_WORK w ON w.FK_CUST_CODE=s.FK_CUST_CODE AND w.FK_SI_ID=s.FK_SI_ID AND w.C_BUSINESS_DATE=@d;

    -- unit ledger (chỉ ngày có cashflow)
    DELETE FROM T_UNIT_LEDGER WHERE C_BUSINESS_DATE=@d;
    INSERT INTO T_UNIT_LEDGER (FK_CUST_CODE,FK_SI_ID,C_BUSINESS_DATE,C_CF_NET,C_DELTA_UNIT,C_UNIT)
    SELECT FK_CUST_CODE,FK_SI_ID,@d,(C_CF_IN-C_CF_OUT),C_DELTA_UNIT,C_UNIT
    ...
    INSERT INTO T_CUSTOMER_NAV_DAILY (C_BUSINESS_DATE,FK_CUST_CODE,FK_SI_ID,C_NAV,C_UNIT,C_UNIT_PRICE,C_DAILY_PNL,C_DAILY_RETURN)
    SELECT @d, FK_CUST_CODE, FK_SI_ID, C_NAV, C_UNIT, C_UNIT_PRICE, C_DAILY_PNL,
