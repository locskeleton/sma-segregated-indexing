...
/*===========================================================================
  ...
  Master suy từ T_INDEXING_PORTFOLIO (sub-account phải đăng ký trước khi ingest).
===========================================================================*/
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- sub-account của event; master suy từ config (T_INDEXING_PORTFOLIO)
        ...
        LEFT JOIN T_INDEXING_PORTFOLIO ip ON ip.C_SI_ACCOUNT = j.C_SI_ACCOUNT AND ip.C_CUST_CODE = @cust;

        IF EXISTS (SELECT 1 FROM @sub WHERE C_MASTER_CODE IS NULL)
            THROW 50022, 'INGEST: sub-account chưa đăng ký (thiếu T_INDEXING_PORTFOLIO cho cust/si_account).', 1;
        ...
        DELETE t FROM T_INDEXING_PORTFOLIO_TICKER t WHERE t.C_SI_ACCOUNT IN (SELECT C_SI_ACCOUNT FROM @sub);
        INSERT INTO T_INDEXING_PORTFOLIO_TICKER (C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE,C_TICKER,C_QUANTITY,C_AVG_COST)
        ...
        INSERT INTO T_CUSTOMER_HOLDING_HIST (C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE,C_TICKER,C_VALID_FROM,C_VALID_TO,C_QUANTITY,C_AVG_COST)
        ...
        UPDATE h SET C_VALID_TO=@d
        FROM T_CUSTOMER_CASH_HIST h JOIN @sub n ON n.C_SI_ACCOUNT=h.C_SI_ACCOUNT
        WHERE h.C_VALID_TO IS NULL AND h.C_CASH<>n.C_CASH;
        INSERT INTO T_CUSTOMER_CASH_HIST (C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE,C_VALID_FROM,C_VALID_TO,C_CASH)
        ...
        INSERT INTO T_CUSTOMER_FEE_INCOME (C_BUSINESS_DATE,C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE,C_TYPE,C_TICKER,C_AMOUNT,C_SOURCE,C_SOURCE_EVENT_ID)
...
BEGIN
    ...
    JOIN T_CUSTOMER_NAV_CURRENT s ON s.C_SI_ACCOUNT=h.C_SI_ACCOUNT
...
BEGIN
    ...
    JOIN (
        ...
        FROM T_CASHFLOW_EVENT WHERE C_BUSINESS_DATE=@d GROUP BY C_SI_ACCOUNT
    ...
    INSERT INTO T_UNIT_LEDGER (C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE,C_BUSINESS_DATE,C_CF_NET,C_DELTA_UNIT,C_UNIT)
    ...
    INSERT INTO T_CUSTOMER_NAV_BALANCE (C_BUSINESS_DATE,C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE,C_NAV,C_UNIT,C_UNIT_PRICE,C_DAILY_PNL,C_DAILY_RETURN)
