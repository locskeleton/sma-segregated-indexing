...
TRUNCATE TABLE T_CUSTOMER_NAV_DAILY;   TRUNCATE TABLE T_SI_NAV_DAILY;
TRUNCATE TABLE T_CUSTOMER_FEE_INCOME;  TRUNCATE TABLE T_SI_INDEX_DAILY;
TRUNCATE TABLE T_SI_HOLDING_DAILY;     TRUNCATE TABLE T_SI_NAV_CURRENT;
...
SELECT s.n, CONCAT('SDI',FORMAT(s.n,'00')), CONCAT(N'Bench SI ',s.n), 'ACTIVE', @d, 0.01, 'VNINDEX'
...
INSERT INTO T_MASTER_PORTFOLIO_TICKER (FK_SI_ID,C_EFFECTIVE_DATE,C_TICKER,C_TARGET_WEIGHT)
SELECT mp.PK_SI_ID, @d, t.tk, 1.0/@nTick
...
/*--- subaccount (KH × SI) ---*/
INSERT INTO T_INDEXING_PORTFOLIO (FK_CUST_CODE,FK_SI_ID,C_JOIN_DATE,C_STATUS,C_MGMT_FEE_RATE)
SELECT CONCAT('KH',FORMAT(c.n,'00000000')), mp.PK_SI_ID, @d, 'ACTIVE', 0.01
FROM #cust c CROSS JOIN T_MASTER_PORTFOLIO mp;

/*--- holdings FO sync (KH × SI × mã) = volume chính ---*/
INSERT INTO T_CUSTOMER_HOLDING_DAILY (C_BUSINESS_DATE,FK_CUST_CODE,FK_SI_ID,C_TICKER,C_QUANTITY,C_AVG_COST)
SELECT @d, ip.FK_CUST_CODE, ip.FK_SI_ID, mpt.C_TICKER, @qty, @price
FROM T_INDEXING_PORTFOLIO ip
JOIN T_MASTER_PORTFOLIO_TICKER mpt ON mpt.FK_SI_ID = ip.FK_SI_ID;

/*--- cash FO sync (=0) + cashflow INITIAL (= giá trị cp để init unit) ---*/
INSERT INTO T_FO_CASH_SYNC (C_BUSINESS_DATE,FK_CUST_CODE,FK_SI_ID,C_CASH)
SELECT @d, FK_CUST_CODE, FK_SI_ID, 0 FROM T_INDEXING_PORTFOLIO;

INSERT INTO T_CASHFLOW_EVENT (FK_CUST_CODE,FK_SI_ID,C_BUSINESS_DATE,C_EVENT_TYPE,C_AMOUNT)
SELECT FK_CUST_CODE, FK_SI_ID, @d, 'INITIAL', @nTick*@qty*@price FROM T_INDEXING_PORTFOLIO;
