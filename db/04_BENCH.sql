...
TRUNCATE TABLE T_SI_ASSET_DAILY;  TRUNCATE TABLE T_MASTER_INDEX_DAILY;
...
/*--- [thin-layer] seed T_SI_ASSET_DAILY (Asset gửi AUM + daily_return): aum = nTick×qty×price, cash 0,
      daily_return NULL (bench 1 phiên), cash_in = INITIAL (khớp cashflow SDI → reconcile sạch). ---*/
INSERT INTO T_SI_ASSET_DAILY (C_BUSINESS_DATE,C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE,C_AUM,C_DAILY_RETURN,C_CASH,C_CASH_IN,C_CASH_OUT)
SELECT @d, C_SI_ACCOUNT, C_CUST_CODE, C_MASTER_CODE, @nTick*@qty*@price, NULL, 0, @nTick*@qty*@price, 0 FROM T_SI_PORTFOLIO;  -- aum; daily_return NULL
