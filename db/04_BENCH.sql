...
TRUNCATE TABLE T_CUSTOMER_NAV_BALANCE;   TRUNCATE TABLE T_MASTER_NAV_BALANCE;
TRUNCATE TABLE T_CUSTOMER_FEE_INCOME;  TRUNCATE TABLE T_MASTER_INDEX_DAILY;
...
TRUNCATE TABLE T_CUSTOMER_HOLDING_HIST; TRUNCATE TABLE T_CUSTOMER_CASH_HIST;
...
SELECT CONCAT('KH',FORMAT(c.n,'00000000')), mp.C_MASTER_CODE,
       CONCAT('SUB',FORMAT(c.n,'00000000'),mp.C_MASTER_CODE), @d, 'ACTIVE', 0.01   -- sub-account code (unique/KH×master)
...
SELECT ip.C_CUST_CODE, ip.C_MASTER_CODE, mpt.C_TICKER, @qty, @price
...
/*--- state seed: cash=0 + watermark C_LAST_SYNC_DATE=@d (bench BỎ QUA ingest per-KH cho scale;
      GATE đếm watermark; interval history do ingest maintain — KHÔNG seed ở bench, EOD compute không cần)
      + cashflow INITIAL (= giá trị cp để init unit) ---*/
INSERT INTO T_CUSTOMER_NAV_CURRENT (C_CUST_CODE,C_MASTER_CODE,C_UNIT,C_CASH,C_PAYABLE_FEE,C_LAST_NAV,C_STATUS,C_LAST_SYNC_DATE)
SELECT C_CUST_CODE, C_MASTER_CODE, 0, 0, 0, 0, 'ACTIVE', @d FROM T_INDEXING_PORTFOLIO;

INSERT INTO T_CASHFLOW_EVENT (C_CUST_CODE,C_MASTER_CODE,C_BUSINESS_DATE,C_EVENT_TYPE,C_AMOUNT)
SELECT C_CUST_CODE, C_MASTER_CODE, @d, 'INITIAL', @nTick*@qty*@price FROM T_INDEXING_PORTFOLIO;
