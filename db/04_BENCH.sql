...
TRUNCATE TABLE T_CUSTOMER_HOLDING_DAILY; TRUNCATE TABLE T_CUSTOMER_CASH_DAILY;
...
/*--- holdings: FO nạp THẲNG vào current (KH × SI × mã) = volume chính. J16 archive sang DAILY trong EOD. ---*/
INSERT INTO T_INDEXING_PORTFOLIO_TICKER (C_CUST_CODE,FK_SI_ID,C_TICKER,C_QUANTITY,C_AVG_COST)
SELECT ip.C_CUST_CODE, ip.FK_SI_ID, mpt.C_TICKER, @qty, @price
...
INSERT INTO T_CUSTOMER_CASH_DAILY (C_BUSINESS_DATE,C_CUST_CODE,FK_SI_ID,C_CASH)
SELECT @d, C_CUST_CODE, FK_SI_ID, 0 FROM T_INDEXING_PORTFOLIO;

INSERT INTO T_CASHFLOW_EVENT (C_CUST_CODE,FK_SI_ID,C_BUSINESS_DATE,C_EVENT_TYPE,C_AMOUNT)
SELECT C_CUST_CODE, FK_SI_ID, @d, 'INITIAL', @nTick*@qty*@price FROM T_INDEXING_PORTFOLIO;
