...
TRUNCATE TABLE T_MASTER_INDEX_DAILY;
TRUNCATE TABLE T_MASTER_HOLDING_BALANCE;     TRUNCATE TABLE T_MASTER_CURRENT;
TRUNCATE TABLE T_EOD_RUN;              TRUNCATE TABLE T_SI_PORTFOLIO_HOLDING;
TRUNCATE TABLE T_SI_CURRENT;       TRUNCATE TABLE T_SI_CASHFLOW_EVENT;
TRUNCATE TABLE T_SI_HOLDING_HIST;
TRUNCATE TABLE T_PRICE_DAILY;          TRUNCATE TABLE T_MASTER_PORTFOLIO_TICKER;
TRUNCATE TABLE T_MASTER_PORTFOLIO_TICKER_HIST;
...
SELECT CONCAT('SDI',FORMAT(s.n,'00')), CONCAT(N'Bench SI ',s.n), 'ACTIVE', @d, 'VNINDEX'
FROM (SELECT TOP (@nSi) ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) n
      FROM sys.all_columns) s;
-- [BRD] không còn T_FEE_CONFIG: phí do Asset gửi.
...
-- HIST là nguồn J12 dựng rổ as-of ⇒ seed PHẢI ghi vào đây; bảng rổ hiện tại chỉ để serve.
INSERT INTO T_MASTER_PORTFOLIO_TICKER_HIST (C_MASTER_CODE,C_TICKER,C_TARGET_WEIGHT,C_CONFIRM_TIME)
SELECT mp.C_MASTER_CODE, t.tk, 1.0/@nTick, CAST(DATEADD(DAY,-1,@d) AS DATETIME2(3))
FROM T_MASTER_PORTFOLIO mp CROSS JOIN #tk t;

INSERT INTO T_MASTER_PORTFOLIO_TICKER (C_MASTER_CODE,C_TICKER,C_TARGET_WEIGHT)
SELECT mp.C_MASTER_CODE, t.tk, 1.0/@nTick
FROM T_MASTER_PORTFOLIO mp CROSS JOIN #tk t;
...
INSERT INTO T_SI_CURRENT (C_SI_ACCOUNT,C_CUST_CODE,C_MASTER_CODE,C_CASH,C_LAST_AUM,C_STATUS,C_LAST_SYNC_DATE)
SELECT C_SI_ACCOUNT, C_CUST_CODE, C_MASTER_CODE, 0, 0, 'ACTIVE', @d FROM T_SI_PORTFOLIO;
...
/*--- [thin-layer] seed THẲNG T_SI_BALANCE (Asset ghi thẳng): aum = nTick×qty×price, cash 0,
      daily_return NULL (bench 1 phiên), cash_in = INITIAL (khớp cashflow SDI → reconcile sạch). ---*/
...
SELECT @d, C_SI_ACCOUNT, C_CUST_CODE, C_MASTER_CODE, @nTick*@qty*@price, NULL, 0, @nTick*@qty*@price, 0 FROM T_SI_PORTFOLIO;  -- aum; daily_return NULL (bench 1 phiên)
