...
TRUNCATE TABLE T_INDEXING_PERFORMANCE_DAILY; TRUNCATE TABLE T_NAV_DAILY;
TRUNCATE TABLE T_SI_INDEX_DAILY;       TRUNCATE TABLE T_SI_HOLDING_DAILY;
TRUNCATE TABLE T_EOD_RUN;              TRUNCATE TABLE T_INDEXING_PORTFOLIO_TICKER;
TRUNCATE TABLE T_POSITION_STATE;       TRUNCATE TABLE T_CASHFLOW_EVENT;
TRUNCATE TABLE T_CUSTOMER_HOLDING_DAILY; TRUNCATE TABLE T_FO_CASH_SYNC;
...
SELECT s.n, CONCAT('SDI',FORMAT(s.n,'00')), CONCAT(N'Bench SI ',s.n), 'ACTIVE', @d, 0.01, 1
...
INSERT INTO T_MASTER_PORTFOLIO_TICKER (C_SI_ID,C_EFFECTIVE_DATE,C_TICKER,C_TARGET_WEIGHT)
SELECT mp.C_SI_ID, @d, t.tk, 1.0/@nTick
...
SELECT c.n, mp.C_SI_ID, @d, 'ACTIVE', 0.01
FROM #cust c CROSS JOIN T_MASTER_PORTFOLIO mp;

/*--- holdings FO sync (KH × SI × mã) = volume chính ---*/
INSERT INTO T_CUSTOMER_HOLDING_DAILY (C_BUSINESS_DATE,C_CUSTOMER_ID,C_SI_ID,C_TICKER,C_QUANTITY,C_AVG_COST)
SELECT @d, ip.C_CUSTOMER_ID, ip.C_SI_ID, mpt.C_TICKER, @qty, @price
FROM T_INDEXING_PORTFOLIO ip
JOIN T_MASTER_PORTFOLIO_TICKER mpt ON mpt.C_SI_ID = ip.C_SI_ID;

/*--- cash FO sync (=0) + cashflow INITIAL (= giá trị cp để init unit) ---*/
INSERT INTO T_FO_CASH_SYNC (C_BUSINESS_DATE,C_CUSTOMER_ID,C_SI_ID,C_CASH)
SELECT @d, C_CUSTOMER_ID, C_SI_ID, 0 FROM T_INDEXING_PORTFOLIO;

INSERT INTO T_CASHFLOW_EVENT (C_CUSTOMER_ID,C_SI_ID,C_BUSINESS_DATE,C_EVENT_TYPE,C_AMOUNT)
SELECT C_CUSTOMER_ID, C_SI_ID, @d, 'INITIAL', @nTick*@qty*@price FROM T_INDEXING_PORTFOLIO;
