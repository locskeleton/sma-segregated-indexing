...
-- ★ AUM=0 → phí ngày = 0 (dòng vẫn tạo, fee 0.00)
INSERT INTO @R SELECT '★ AUM=0 → phí ngày = 0',
  CASE WHEN (SELECT COUNT(*) FROM T_SI_FEE_BALANCE WHERE C_SI_ACCOUNT='SUBZERO')=1
        AND (SELECT C_FEE_AMOUNT FROM T_SI_FEE_BALANCE WHERE C_SI_ACCOUNT='SUBZERO')=0 THEN 1 ELSE 0 END,
...
==============================================================================*/
INSERT INTO @R SELECT 'UDF_SI_FEE_ACCRUING SUBX = 123287.70 (kỳ 202605 chưa chốt)',
  CASE WHEN dbo.UDF_SI_FEE_ACCRUING('SUBX') = 123287.70 THEN 1 ELSE 0 END,
  CONCAT('accruing=', dbo.UDF_SI_FEE_ACCRUING('SUBX'));
INSERT INTO @R SELECT 'UDF_SI_FEE_DEBT SUBX = 205479.70 (nợ+tích lũy)',
  CASE WHEN dbo.UDF_SI_FEE_DEBT('SUBX') = 205479.70 THEN 1 ELSE 0 END,
