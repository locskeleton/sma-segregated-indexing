...
DELETE FROM T_SI_FEE_CHARGE; DELETE FROM T_SI_FEE_DAILY; DELETE FROM T_SI_FEE_RATE; DELETE FROM T_FEE_CONFIG;
...
INSERT INTO @R SELECT 'ACCRUE SUBONE 1 dòng days=1 kỳ202604',
  CASE WHEN (SELECT COUNT(*) FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBONE')=1
        AND (SELECT C_DAYS FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBONE')=1
        AND (SELECT C_PERIOD FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBONE')='202604'
        AND (SELECT ROUND(C_FEE_AMOUNT,6) FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBONE')=@feeDay THEN 1 ELSE 0 END,
  (SELECT CONCAT('days=',C_DAYS,' fee=',C_FEE_AMOUNT) FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBONE');

-- SUBFRI: 1 dòng, days=3, kỳ 202604, fee = 3×feeDay
INSERT INTO @R SELECT 'ACCRUE SUBFRI 1 dòng days=3 (T6+T7+CN)',
  CASE WHEN (SELECT COUNT(*) FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBFRI')=1
        AND (SELECT C_DAYS FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBFRI')=3
        AND (SELECT ROUND(C_FEE_AMOUNT,6) FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBFRI')=ROUND(3*@feeDay,6) THEN 1 ELSE 0 END,
  (SELECT CONCAT('days=',C_DAYS,' fee=',C_FEE_AMOUNT) FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBFRI');

-- ★ SUBX: 2 dòng (case 30/4-1/5)
INSERT INTO @R SELECT '★ ACCRUE SUBX TÁCH 2 dòng (vắt tháng)',
  CASE WHEN (SELECT COUNT(*) FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBX')=2 THEN 1 ELSE 0 END,
  CONCAT('count=',(SELECT COUNT(*) FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBX'));
-- dòng kỳ 202604: business=Apr29, days=2
INSERT INTO @R SELECT '★ SUBX kỳ202604 business=Apr29 days=2',
  CASE WHEN EXISTS (SELECT 1 FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBX' AND C_PERIOD='202604'
        AND C_BUSINESS_DATE='2026-04-29' AND C_DAYS=2 AND ROUND(C_FEE_AMOUNT,6)=ROUND(2*@feeDay,6)) THEN 1 ELSE 0 END,
  (SELECT CONCAT('biz=',CONVERT(VARCHAR,C_BUSINESS_DATE,23),' days=',C_DAYS,' fee=',C_FEE_AMOUNT)
   FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBX' AND C_PERIOD='202604');
-- dòng kỳ 202605: business=May1, days=3
INSERT INTO @R SELECT '★ SUBX kỳ202605 business=May01 days=3',
  CASE WHEN EXISTS (SELECT 1 FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBX' AND C_PERIOD='202605'
        AND C_BUSINESS_DATE='2026-05-01' AND C_DAYS=3 AND ROUND(C_FEE_AMOUNT,6)=ROUND(3*@feeDay,6)) THEN 1 ELSE 0 END,
  (SELECT CONCAT('biz=',CONVERT(VARCHAR,C_BUSINESS_DATE,23),' days=',C_DAYS,' fee=',C_FEE_AMOUNT)
   FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBX' AND C_PERIOD='202605');
-- tổng ngày 2 dòng = 5 (= dải [Apr29,May4))
INSERT INTO @R SELECT '★ SUBX Σdays=5 (khớp dải look-forward)',
  CASE WHEN (SELECT SUM(C_DAYS) FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBX')=5 THEN 1 ELSE 0 END,
  CONCAT('Σdays=',(SELECT SUM(C_DAYS) FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBX'));
...
INSERT INTO @R SELECT 'CLOSE daily 202604=CLOSED, 202605=ACCRUED',
  CASE WHEN (SELECT COUNT(*) FROM T_SI_FEE_DAILY WHERE C_PERIOD='202604' AND C_STATUS<>'CLOSED')=0
...
INSERT INTO @R SELECT 'HOOK ASSET_NAV READY → accrue tự fire',
  CASE WHEN @ec=0
        AND EXISTS (SELECT 1 FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBHOOK' AND C_ACCRUED_ON='2026-06-15'
              AND C_PERIOD='202606' AND C_DAYS=1) THEN 1 ELSE 0 END,
  CONCAT('err=',@ec,' rows=',(SELECT COUNT(*) FROM T_SI_FEE_DAILY WHERE C_SI_ACCOUNT='SUBHOOK'));
