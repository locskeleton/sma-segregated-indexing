...
- **`T_FEE_CONFIG`** (**PK (`C_MASTER_CODE`, `C_FEE_TYPE`)** + GUID `PK_FEE_CONFIG`; `C_FEE_TYPE`[MGMT_FEE|TAX|PERF_FEE|DIVIDEND|CUSTODY_FEE|…], `C_FEE_GROUP`[INCOME|PAYABLE], `C_RATE` (%/năm, **NULL-able** — NULL = không accrue), `C_DAY_COUNT` (default 365), `C_UPDATED_BY`/`C_UPDATED_TIME`) — **CATALOG CHÍNH SÁCH PHÍ/THUẾ chung cấp master** (nguồn quản lý/cài đặt chính sách phí, thuế hệ thống; không chỉ phí accrue). **`C_FEE_TYPE` + `C_FEE_GROUP` DÙNG CHUNG vocabulary với `T_SI_FEE_LEDGER` — mã phí + nhóm phải KHỚP giữa 2 bảng.** J06 **chỉ accrue** dòng `C_FEE_GROUP='PAYABLE' AND C_RATE>0` (phí phải trả có rate); loại INCOME (vd cổ tức) hoặc `C_RATE` NULL (point-event custody/thuế GD) KHÔNG accrue. **Thêm chính sách phí mới = INSERT 1 dòng, KHÔNG sửa schema/SP.** Loại phí trừ thẳng cash (custody/thuế GD) khai với `C_RATE` NULL.
- **`T_MASTER_PORTFOLIO_TICKER`** (`C_MASTER_CODE`, effective_date, ticker; target_weight) — **FO tính & feed**; Σ = 100% cổ phiếu/eff_date.
- **`T_SI_PORTFOLIO`** (`C_SI_ACCOUNT` UNIQUE; `C_CUST_CODE`, `C_MASTER_CODE`, sub_account_no, join_date, status, close_date, initial_amount, sip_amount, sip_schedule, min_invest) — registry tiểu khoản + cấu hình đầu tư KH (FR-04). **Phí KHÔNG cấp si** — chính sách phí cấp master (`T_FEE_CONFIG`).
...
- **`T_SI_FEE_LEDGER`** (fee_ledger_id BIGINT PK; business_date, `C_SI_ACCOUNT`, group[INCOME|PAYABLE], type[DIVIDEND|CUSTODY_FEE|MGMT_FEE|TAX|PERF_FEE|…], ticker[opt], period[opt], amount, source[FO|BO], source_event_id [dedup], created_time) — **SỔ CÁI phí/thu nhập per-KH (sparse, idempotent)** gộp cổ tức + phí lưu ký + phí ACCRUE đã cắt. DIVIDEND→group INCOME (cộng tài sản); CUSTODY_FEE + các loại phí cắt (MGMT_FEE/TAX/PERF_FEE…)→group PAYABLE (SUM theo group = tổng nhóm). Dòng tiền/sự kiện ngoài, KHÔNG derive được → capture lúc phát sinh cho **báo cáo tài sản FR-06**. **2 đường ghi**: FO đẩy DIVIDEND/CUSTODY_FEE (source FO, qua `SP_INGEST_CUSTOMER`); **phí ACCRUE do BO cắt** → type theo `fee_type` của charge (default MGMT_FEE; source BO, qua `SP_INGEST_FEE_CHARGE` → `payable −= amount` net-off, trừ tổng mọi loại; thiếu đủ cứ trừ, residual carry; **KHÔNG status lifecycle** — BO sở hữu việc cắt, SDI chỉ net-off). Idempotent qua `source_event_id`. (Accrual phí hằng ngày VẪN ở `T_SI_NAV_CURRENT.C_PAYABLE_FEE` — ledger chỉ chứa event ĐÃ phát sinh.)
- **`T_SI_FEE_ACCRUAL`** (**PK clustered (`C_SI_ACCOUNT`, `C_FEE_TYPE`, `C_BUSINESS_DATE`)** + GUID `PK_…`; `C_BUSINESS_DATE`, `C_SI_ACCOUNT`, `C_CUST_CODE`, `C_MASTER_CODE`, `C_FEE_TYPE`, `C_ACCRUAL_AMOUNT DECIMAL(20,6)`; DATA_COMPRESSION=PAGE; index `IX_SI_FEE_ACCRUAL_ACCT (C_SI_ACCOUNT, C_BUSINESS_DATE)` INCLUDE type/amount) — **LOG lượng phí ACCRUE (trích trước) per (sub-account × loại phí × NGÀY)**. Ghi ở **J06b** trong `SP_EOD_COMPUTE` (sau bước accrue `C_PAYABLE_FEE`; idempotent: DELETE @p_d trước INSERT). `C_PAYABLE_FEE` (tổng mọi loại) vẫn ở `T_SI_NAV_CURRENT`/`T_SI_NAV_BALANCE` cho NAV; bảng này cho **BREAKDOWN per-type** phục vụ sao kê NAV gửi KH (FR-06 RS5): `pending(loại,≤asOf) = Σ C_ACCRUAL_AMOUNT(loại,≤asOf) − Σ cắt(loại,≤asOf từ T_SI_FEE_LEDGER group PAYABLE)` (exact as-of, cả 2 là tổng event, KHÔNG dùng proportion); Σ mọi loại = `C_PAYABLE_FEE` (khớp tổng).
...
- *(Đã BỎ `T_SDI_CONFIG`)* — phí ACCRUE chuyển sang **config-driven đa-loại** ở `T_FEE_CONFIG`: loại phí nào accrue (group=PAYABLE + rate>0) + rate + day_count (default 365) khai cấp master; basis = AUM gross. Không còn toggle/knob hardcode.
...
| **J6** | `ACCRUE_FEE` *(chạy trong J07)* | J07 | payable state + `T_FEE_CONFIG` (master) | accrue payable += AUM×(ngày dương lịch)×Σ(rate/day_count) cho dòng `C_FEE_GROUP='PAYABLE' AND C_RATE>0` của master (config-driven; không config/NULL-rate/INCOME→không accrue). Net-off riêng khi nhận **event BO cắt phí** (Kafka → `SP_INGEST_FEE_CHARGE`, ngoài batch) | ✅ | ‖ | – |
| **J6b** | `LOG_ACCRUAL` *(chạy trong J07, sau J6)* | J6 | accrue per-type @d | **`T_SI_FEE_ACCRUAL`** log lượng accrue per (sub-account × loại phí × NGÀY) — breakdown cho FR-06 RS5 (kê từng khoản phải trả). Idempotent: DELETE @p_d trước INSERT | ✅ | ‖ | – |
| **INGEST-FEE** | `SP_INGEST_FEE_CHARGE` (Kafka, event BO cắt phí, ngoài batch) | — | event {si_account, amount, charge_date, fee_type?, period?} | `payable −= amount` (net-off, trừ tổng mọi loại) + log `T_SI_FEE_LEDGER` (group PAYABLE, type theo fee_type [default MGMT_FEE]; dedup source_event_id) | ✅ | ‖ | ✅ (dedup) |
...
| **J11** | `SI_AGG` tổng hợp master | J8, J10 | cash/pending/div/stock/NAV/unit per tiểu khoản + T_SI_FEE_LEDGER + T_SI_CASHFLOW_EVENT | **T_MASTER_NAV_BALANCE** (composition + NAV + hiệu suất + cổ tức/phí Σ + **[PM] cash_in/cash_out Σ + total_account + total_asset gồm receivables**) + **T_MASTER_NAV_CURRENT** (upsert) | ✅ | ‖ | – |
...
| FR-04 Thông tin đầu tư | GET /customer/{id}/si/{si}/info | `SP_GET_SI_INFO` | T_SI_PORTFOLIO + master (`C_MGMT_FEE_RATE_EFFECTIVE` đọc từ `T_FEE_CONFIG`, type MGMT_FEE) |
...
| FR-06 Báo cáo tài sản | GET /customer/{id}/si/{si}/asset-report | `SP_GET_ASSET_REPORT` | T_SI_NAV_BALANCE (NAV+payable) + T_SI_FEE_LEDGER (cổ tức/phí lưu ký group INCOME/PAYABLE + phí BO cắt mọi loại group PAYABLE) + `C_FEE_ACCRUED_TOTAL` (tổng phí phải trả accrued chưa cắt, RS4 chi tiết) + cash/stock reconstruct (T_SI_CASH_HIST + T_SI_HOLDING_HIST×giá theo interval) + **T_SI_FEE_ACCRUAL** (RS5 kê từng khoản phải trả per-type) |

> **FR-06 result sets:** RS1 summary (NAV + payable + cổ tức/phí lưu ký lũy kế), RS2 holdings @asOf, RS3 chi tiết INCOME (cổ tức), RS4 chi tiết PAYABLE đã cắt, **RS5 kê từng khoản phải trả per-type** — cột `C_FEE_TYPE`, `C_FEE_ACCRUED` (Σ accrue ≤asOf từ `T_SI_FEE_ACCRUAL`), `C_FEE_PAID` (Σ cắt ≤asOf từ `T_SI_FEE_LEDGER` group PAYABLE), `C_FEE_PENDING` (= accrued − paid). Exact as-of (cả 2 là tổng event, KHÔNG dùng proportion); Σ pending mọi loại = `C_PAYABLE_FEE`. Cho sao kê NAV gửi KH kê TỪNG khoản phải trả chính xác @asOf.
