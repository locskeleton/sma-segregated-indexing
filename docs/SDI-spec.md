...
| **INGEST-FEE** | `SP_INGEST_FEE_CHARGE` (Kafka, event BO cắt phí, ngoài batch) | — | event {si_account, amount, charge_date, fee_type?} | `payable −= amount` (net-off, trừ tổng mọi loại) + log `T_SI_FEE_LEDGER` (group PAYABLE, type theo fee_type [default MGMT_FEE]; dedup source_event_id) | ✅ | ‖ | ✅ (dedup) |
