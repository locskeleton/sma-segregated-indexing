...
>
> **Master-level (luồng 8b/8c):** **`SP_GET_ASSET_MASTER_SNAPSHOT @p_business_date,@p_mode`** (db/05_API.sql) — 1 payload JSON/master (key=`C_MASTER_CODE`): master nav/unit/up/return + total_asset/cash breakdown + `index_value` (PR) + `benchmark_value` (PR). Cho Asset dựng overview/AUM (8b = ngày mới nhất) + chuỗi chart FR-03 (master TR + index + benchmark). Cũng RECONSTRUCT-ONLY từ `T_MASTER_NAV_BALANCE`+`T_MASTER_INDEX_DAILY`+`T_BENCHMARK_DAILY` (DATED) ⇒ EOD/HISTORY replay y hệt. Master-level CÓ pending/div hist (khác per-KH). DRAFT payload.
