...
| 🟠 (sparse) | `T_SI_CASHFLOW_EVENT`, `T_SI_FEE_INCOME`, `T_SI_UNIT_LEDGER` | theo sự kiện | tần suất nạp/rút/cổ tức |
...
| Kịch bản (KH × master/KH × mã/master) | small 1.000×3×20 · medium 10.000×5×25 · large 50.000×5×25 |
| Tiểu khoản S₀ = KH×master | small **3.000** · medium **50.000** · large **250.000** |
| Holdings H₀ = KH×master×mã | small 60K · medium 1,25M · large 6,25M |
...
## 4. Driver #2: `T_SI_HOLDING_HIST` (interval) — full history, tăng theo CHURN
...
**So với snapshot dated cũ (đã bỏ):** snapshot = H₀ × 252/năm. medium 10 năm snapshot ≈ **3,15 tỷ** dòng; interval churn-4 ≈ **51M** → **giảm ~98% STORAGE**, mà vẫn **full history** (tái dựng mọi ngày qua `valid_from≤D<valid_to`). Event không có biến động: DIFF (EXCEPT) thấy 0 thay đổi ⇒ **0 dòng ghi** (storage đứng yên). **Lưu ý THỜI GIAN:** DIFF quét **current ⋈ open-rows (2×H₀) mỗi lần ingest** dù 0 thay đổi → chi phí DIFF theo **số event ingest**, không theo ngày. DIFF này chạy **TẠI INGEST (per-event Kafka), KHÔNG nằm trong EOD** → EOD core nhẹ hẳn (xem §7).
