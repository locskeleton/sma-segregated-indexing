...
|---|---|---|---|
| 🔴 **GROW không chặn (dense/ngày)** | `T_CUSTOMER_NAV_DAILY` | +1 dòng/tiểu khoản/EOD (history NAV/unit) | **ngày × KH** ← driver chính (cho FR-03) |
...
→ **Hai driver dài hạn:** (1) `T_CUSTOMER_NAV_DAILY` dense theo ngày×KH (lớn nhất); (2) interval hist theo **churn** — full history nhưng tăng chậm vì danh mục index ít biến động. Bảng 🟢 chỉ to theo số KH.
...
| Bytes/row (raw): nav_daily ~50 · hist ~64 | nén CCI/PAGE prod **~3–5×** |

> nav_daily tích luỹ ≈ S₀ × phiên × hệ_số_TB. Interval hist tích luỹ ≈ H₀ (base mở) + H₀ × churn × năm × hệ_số_TB.

---

## 3. Driver #1: `T_CUSTOMER_NAV_DAILY` — số bản ghi tích luỹ (triệu dòng)
...
**Dung lượng** (raw ~50B/row; nén ~÷4): medium 1Y nóng ~1,3 GB raw / ~0,3 GB nén; large 1Y nóng ~6,5 GB / ~1,6 GB. → Khiêm tốn (nav_daily = holdings ÷ số mã).
...
> `customer_nav_daily` 10 năm ~2,5 tỷ (khớp [SDI-spec §11]) — khối lớn nhất, cần partition/CCI/điểm thưa. `customer_holding_hist` full history nhưng nhờ interval chỉ ~1 tỷ (churn 4/y) thay vì ~63 tỷ nếu snapshot dated → tiết kiệm ~98%, vẫn đủ tái dựng mọi ngày.
...
- Sparse (cashflow, cổ tức/phí, unit_ledger) chưa gộp — theo lịch sự kiện, thường << nav_daily.
